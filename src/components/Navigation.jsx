...
export function Navigation() {
  ...
  const mobListRef = useRef(null);
  const mobIconRef = useRef(null);

  // Lock overflow (scrolling) when menu is open
  useEffect(() => {
  ...
  // Close menu with ESC key and focusing to th right element
  useEffect(() => {
    if (!isOpen) return;
    const onKeyDown = (e) => {
      if (e.key === "Escape") {
        setIsOpen(false);
        mobIconRef.current?.focus();
      }
    };

    document.addEventListener("keydown", onKeyDown);
    mobListRef.current?.focus();
    return () => document.removeEventListener("keydown", onKeyDown);
  }, [isOpen]);

  return (
    <header className="mb-14 mx-6 mt-10 md:mx-12 md:mb-16 xl:mt-14 xl:mb-30 xl:mx-auto xl:w-277.5">
      <nav className="flex justify-between items-center">
        {logo}
        {/* Mobile Menu button */}
        <button
          className="cursor-pointer xl:hidden xl:invisible"
        ...
        <div
          className={`fixed inset-0 top-16 bg-gradient-1 transition-all duration-300 z-1  ${isOpen ? "opacity-100 visible" : "opacity-0 invisible"} `}
          onClick={() => setIsOpen(false)}
        >
          {/* mob list */}
          <div
            ref={mobListRef}
            className={`fixed py-10 top-26 left-6 right-6 md:left-12 md:right-12 bg-white rounded-sm transition-all duration-300 ease-in-out z-1 ${isOpen ? "translate-x-0" : "translate-x-[120%]"}`}
          >
            <List
        ...
        <Button style="orangeBtn hidden invisible xl:flex xl:visible">
          Get Started
        </Button>
      </nav>
    </header>
